...
class CommentServiceTest {
    ...
    void write() {
        ...
        User givenUser = User.builder().id(1L).name("test1").email(new Email("test1@gmail.com")).password("1234").build();
        Comment givenComment = Comment.builder().id(1L).content(randomAlphabetic(20)).build();
        givenUser.addComment(givenComment);

        given(postRepository.findByIdCustom(1L,2L,1L)).willReturn(Optional.ofNullable(givenPost));
        given(commentRepository.save(Comment.builder().content(randomAlphabetic(20)).build())).willReturn(givenComment);

        commentService.write(1L, 2L, 1L, Comment.builder().content(randomAlphabetic(20)).build());
        ...
        assertEquals(givenPost.getComments().size(),1);
    ...
    void findAll () {
        ...
        User user1 = User.builder().id(1L).name("test1").email(new Email("test1@gmail.com")).password("1234").build();
        user1.addPost(post1);
        Comment comment = Comment.builder().id(1L).content(randomAlphabetic(20)).build();
        user1.addComment(comment);

        List<Comment> givenComments = new ArrayList<>();
        givenComments.add(comment);

        given(postRepository.findByIdCustom(1L,2L,1L)).willReturn(Optional.ofNullable(post1));
        given(commentRepository.findByPost_IdOrderByIdDesc(1L)).willReturn(givenComments);

        commentService.findAll(1L,2L,1L);

        then(postRepository).should(times(1)).findByIdCustom(any(),any(),any());
        then(commentRepository).should(times(1)).findByPost_IdOrderByIdDesc(any());
    }
