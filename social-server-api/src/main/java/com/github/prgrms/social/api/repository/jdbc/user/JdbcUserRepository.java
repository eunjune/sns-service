/*
package com.github.prgrms.social.api.repository.jdbc.user;
...
public class JdbcUserRepository implements UserRepository {
    ...
    public User save(User user) {
        ...
        return User.builder()
                ...
                .profileImageUrl(user.getProfileImageUrl().orElse(null))
                .loginCount(user.getLoginCount())
                .createAt(user.getCreateAt())
                .lastLoginAt(user.getLastLoginAt().orElse(null))
                .build();
    }

    @Override
    public void update(User user) {
        jdbcTemplate.update("UPDATE users SET name=?,passwd=?,profile_image_url=?,login_count=?,last_login_at=? WHERE seq=?",
                user.getName(),
                user.getPassword(),
    ...
    public List<ConnectedUser> findAllConnectedUser(Id<User, Long> userId) {
        */
/*return jdbcTemplate.query("SELECT u.seq,u.name,u.email,u.profile_image_url,c.granted_at FROM connections c JOIN users u ON c.target_seq=u.seq WHERE c.user_seq=? AND c.granted_at is not null ORDER BY seq DESC",
                new Object[]{userId.getValue()},
